...
package net.mcreator.utopiacraft.world.dimension;
...
import net.mcreator.utopiacraft.item.UtopiaDimensionItem;
import net.mcreator.utopiacraft.block.AugmentedIronBlockBlock;
...
public class UtopiaDimensionDimension extends UtopiacraftModElements.ModElement {
	@ObjectHolder("utopiacraft:utopiadimension_portal")
	public static final CustomPortalBlock portal = null;
	public UtopiaDimensionDimension(UtopiacraftModElements instance) {
		super(instance, 7);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("utopiacraft:utopiadimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("utopiadimension_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("utopiadimension_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("utopiadimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new UtopiaDimensionItem().setRegistryName("utopiadimension"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					...
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("utopiacraft:utopiadimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == AugmentedIronBlockBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? AugmentedIronBlockBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, AugmentedIronBlockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
